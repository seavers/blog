...
```text
TOTP = Truncate(HMAC-SHA-1(K, C)) mod 10^Digits
```

其中各个参数的含义与计算流程如下：

1. **共享密钥（Secret Key，即 `K`）**：由服务端生成并通过 Base32 编码（如 `JBSWY3DPEHPK3PXP`）。在客户端计算前需将其还原解码为原始的二进制字节数组。
2. **时间步长计数器（Time-Step Counter，即 `C`）**：
   ```text
   C = floor((T - T0) / X)
   ```
   - `T` 为当前系统的 Unix 时间戳（秒）；
   - `T0` 为时间起点（标准规定为 0，即 1970-01-01 00:00:00 UTC）；
   - `X` 为步长周期（默认通常为 30 秒）。
...
4. **动态截断（Dynamic Truncation）**：
   - 取 HMAC 输出的最后一个字节（第 19 字节）的低 4 位（`hmac[19] & 0x0F`）作为偏移量 `Offset`（取值范围 0~15）；
   - 从 HMAC 结果的第 `Offset` 字节开始连续取出 4 个字节；
   - 屏蔽最高位符号位（避免负数运算），拼装成一个 31 位的无符号整数；
   - 对 `10^Digits`（如 6 位即模 1000000）求模，不足位数时左侧补 0，生成最终呈现给用户的 6 位或 8 位数字口令。
